    ...
    <form wire:submit.prevent="update" class="space-y-6">
        ...
        <div class="mb-6">
            <figure class="relative">
                <div class="aspect-[16/9] w-full bg-gray-100 rounded-lg overflow-hidden relative border-2 border-dashed border-gray-300">
                    @if ($image)
                        <!-- Nueva imagen cargada (preview) -->
                        <img class="w-full h-full object-cover"
                            src="data:{{ $image->getMimeType() }};base64,{{ base64_encode($image->get()) }}"
                            alt="Nueva imagen del producto">

                        <div class="absolute top-2 left-2 bg-blue-600 text-white px-3 py-1 rounded-full text-xs font-medium">
                            Nueva imagen
                        </div>

                        <!-- Overlay con botones -->
                        <div class="absolute inset-0 bg-black bg-opacity-0 hover:bg-opacity-30 transition-all duration-200 flex items-center justify-center group">
                            <div class="opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex gap-2">
                                <label class="px-4 py-2 bg-blue-600 text-white rounded-lg cursor-pointer hover:bg-blue-700 transition-colors flex items-center shadow-lg">
                                    ...
                                    Cambiar
                                    <input type="file" class="hidden" accept="image/*" wire:model="image" wire:loading.attr="disabled">
                                </label>
                    ...
                    @elseif ($current_image_path)
                        ...
                        <img class="w-full h-full object-cover"
                            src="{{ Storage::url($current_image_path) }}"
                    ...
                    @else
                        <!-- No hay imagen -->
                        <div class="w-full h-full flex flex-col items-center justify-center text-gray-400 hover:text-gray-600 transition-colors">
                            ...
                            <p class="text-sm text-center mb-4 max-w-xs">Sube una imagen en formato JPG, PNG o GIF. Tamaño máximo 2MB.</p>

                            <label class="px-6 py-3 bg-blue-600 text-white rounded-lg cursor-pointer hover:bg-blue-700 transition-colors flex items-center shadow-md">
        ...
        <div>
            ...
            <input type="text" wire:model="productData.sku" id="sku"
        ...
        <div>
            ...
            <input type="text" wire:model="productData.name" id="name"
                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input type="number" wire:model="productData.price" id="price" step="0.01" min="0.01"
        ...
        @if ($this->categories->count())
            <div>
                ...
                <x-select wire:model.live="category_id" id="category_id" class="w-full" required>
        ...
        @if ($this->subcategories->count())
            <div>
                ...
                <x-select wire:model="productData.subcategory_id" id="subcategory_id" class="w-full" required>
                    <option value="">Seleccionar subcategoría</option>
                    @foreach ($this->subcategories as $subcategory)
                        <option value="{{ $subcategory->id }}">{{ $subcategory->name }}</option>
                    @endforeach
                </x-select>
                @error('productData.subcategory_id')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
        @endif
        ...
        <div>
            ...
            <textarea wire:model="productData.description" id="description" rows="4"
                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                placeholder="Descripción detallada del producto" required></textarea>
        ...
        <div class="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
            ...
            <div class="text-xs text-yellow-700 space-y-1">
                <p><strong>SKU:</strong> "{{ $productData['sku'] ?? 'vacío' }}"</p>
                <p><strong>Name:</strong> "{{ $productData['name'] ?? 'vacío' }}"</p>
                <p><strong>Price:</strong> "{{ $productData['price'] ?? 'vacío' }}"</p>
                <p><strong>Family ID:</strong> {{ $family_id ?: 'no seleccionado' }}</p>
                <p><strong>Category ID:</strong> {{ $category_id ?: 'no seleccionado' }}</p>
                <p><strong>Subcategory ID:</strong> {{ $productData['subcategory_id'] ?? 'no seleccionado' }}</p>
                <p><strong>Nueva imagen:</strong> {{ $image ? 'Sí' : 'No' }}</p>
                <p><strong>Imagen actual:</strong> {{ $current_image_path ? 'Sí' : 'No' }}</p>
            </div>
        </div>
